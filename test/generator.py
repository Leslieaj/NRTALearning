...
class RTAGenerator:
    ...
    def random_states(self, statesnumber):
        ...
        acceptsize = random.randint(floor,int(math.floor(statesnumber / 4.0))+1)
    ...
    def random_intervals(self, partitionsize):
        ...
        intervals_num =random.randint(0, partitionsize)

        odd_index = []
        even_index = []
        count = 0
        while count < intervals_num:
            endpoint1 = random.randint(0,20)
            endpoint2 = random.randint(0,20)
            endponit_pair = [endpoint1, endpoint2]
            endponit_pair.sort()
            odd_index.append(endponit_pair[0])
            even_index.append(endponit_pair[1])
            count = count + 1
        if intervals_num == 0:
            intervals = []
        else:
            ...
            if flag == 0:
                ...
                left = random.sample(['[','('], 1)
                right = random.sample([']',')'], 1)
                if odd_index[intervals_num-1] == even_index[intervals_num-1]:
                    left = '['
                    right = ']'
...
def buildjson(g, filename):
    ...
    for t in g.tran:
        t_dict = t.show()
        (key, values), = t_dict.items()
        tran_dict[key] = values
    
    gdict = {"name":g.name, "l":g.s, "sigma":g.sigma, "tran":tran_dict, "init":g.init, "accept":g.accept}
